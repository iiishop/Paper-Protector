...
bool MoistureSensor::begin() {
    ...
    digitalWrite(_irledPin, LOW); // 初始状态关闭
...
MoistureSensor::MeasurementResult MoistureSensor::measure() {
    ...
    for (int retry = 0; retry < MAX_RETRIES; retry++) {
        ...
        if (!_sensor.readAllChannels(readings)) {
            digitalWrite(_irledPin, LOW); // 确保关闭LED
            // 读取失败，重试
            if (retry < MAX_RETRIES - 1) {
                delay(RETRY_DELAY_MS);
            }
...
as7341_gain_t MoistureSensor::decreaseGain(as7341_gain_t currentGain) {
    // 降低增益到下一个较低的级别
    switch (currentGain) {
        case AS7341_GAIN_512X: return AS7341_GAIN_256X;
        case AS7341_GAIN_256X: return AS7341_GAIN_128X;
        case AS7341_GAIN_128X: return AS7341_GAIN_64X;
        case AS7341_GAIN_64X:  return AS7341_GAIN_32X;
        case AS7341_GAIN_32X:  return AS7341_GAIN_16X;
        case AS7341_GAIN_16X:  return AS7341_GAIN_8X;
        case AS7341_GAIN_8X:   return AS7341_GAIN_4X;
        case AS7341_GAIN_4X:   return AS7341_GAIN_2X;
        case AS7341_GAIN_2X:   return AS7341_GAIN_1X;
        case AS7341_GAIN_1X:   return AS7341_GAIN_0_5X;
        case AS7341_GAIN_0_5X: return AS7341_GAIN_0_5X; // 已经是最低增益
        default: return currentGain;
    }
}
    ...
    if (_instance) {
        ...
        if (gainStr != nullptr) {
            ...
            if (colonPos != nullptr) {
                ...
                as7341_gain_t gain;
                switch (gainValue) {
                    case 0:   gain = AS7341_GAIN_0_5X; break;
                    case 1:   gain = AS7341_GAIN_1X; break;
                    case 2:   gain = AS7341_GAIN_2X; break;
                    case 4:   gain = AS7341_GAIN_4X; break;
                    case 8:   gain = AS7341_GAIN_8X; break;
                    case 16:  gain = AS7341_GAIN_16X; break;
                    case 32:  gain = AS7341_GAIN_32X; break;
                    case 64:  gain = AS7341_GAIN_64X; break;
                    case 128: gain = AS7341_GAIN_128X; break;
                    case 256: gain = AS7341_GAIN_256X; break;
                    case 512: gain = AS7341_GAIN_512X; break;
                    default:  gain = AS7341_GAIN_128X; break; // 默认值
                }
